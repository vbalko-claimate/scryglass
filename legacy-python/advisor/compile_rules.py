...
def _build_compile_prompt(deck_path: str, deck_name: str,
    ...
    return "\n".join([
        ...
        "",
        "=== HEURISTICS ENGINE (do NOT duplicate) ===",
        "",
        "A separate heuristics engine handles these AUTOMATICALLY at runtime:",
        "- Mulligan: land count, color availability, tapped lands, early play assessment",
        "- Removal: best target, ward costs, hexproof/shroud detection, aura removal",
        "- Combat: lethal detection, double strike, blocker assignment",
        "- Creatures: curve-out order, evasion on empty board, flash hold vs deploy",
        "- Safety: mana verification, attack validation, protection warnings",
        "",
        "Do NOT create rules for generic mulligan (land counts), removal targeting,",
        "combat math, or creature deployment order. Focus on DECK-SPECIFIC knowledge:",
        "- Card synergies and combos unique to this deck",
        "- Sequencing that requires deck knowledge (e.g. play enchantment before creature)",
        "- Matchup-specific gameplans",
        "- Mulligan rules only for deck-specific key cards (e.g. 'keep if Healer's Hawk + aura')",
        "",
        "=== RULES ===",
        "",
        "IMPORTANT:",
        ...
        "- Mulligan rules: only for deck-specific combos, NOT generic land/spell counts",
        ...
        "- keyword matcher supports pipe: \"destroy|exile|damage\"",
        "- castable checks both CMC and color match against untapped lands",
        "- power_min/toughness_min use LIVE stats (including buffs from auras)",
        "",
